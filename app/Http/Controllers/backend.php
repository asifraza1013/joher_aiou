...
class backend extends Controller
{
    ...
    public function alltdata($id)
    {
        ...
        return view('front.pages.all-thesis', compact('class', 'thesis', 'description'));
    }
    public function allgdata($id)
    {
        ...
        return view('front.pages.all-guess', compact('class', 'guess', 'description'));
    }
    public function allvdata($id)
    {
        ...
        return view('front.pages.all-lectures', compact('class', 'book', 'description'));
    }
    ...
    {
        if(is_null($request->username) || is_null($request->comment)){
            return redirect()->back()->with('error', 'Pleaes try with correct data');
        }
        $auth = Auth::user();
        $comment = new comment();

        $comment->uname = $request->username;
        $comment->comment = $request->comment;
        if (!empty($auth) && $auth->role == 'admin') {
            $comment->is_admin = true;
        }
        $comment->save();
        return redirect()->back()->with('success', 'You comment submited successfully. Soon we will get back to you! thank you.');
    }

    public function addreply(Request $request)
    {
        if(is_null($request->username) || is_null($request->reply) || is_null($request->cmntid)){
            return redirect()->back()->with('error', 'Pleaes try with correct data');
        }

        $reply = new reply();
        $reply->uname = $request->username;
        ...
        return redirect()->back()->with('success', 'You reply submited successfully. Soon we will get back to you! thank you.');
    }
}
